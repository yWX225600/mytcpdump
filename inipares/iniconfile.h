...
/**
 * 获取config段名下的所有key-value
 * @param config 配置对象指针
 * @param section　段名,没有段名时可以为NULL
 * @return 成功返回INI_CONFIG指针,失败返回null,用户不用释放指针
 */
INI_CONFIG* ini_config_get_allvalue_from_section(INI_CONFIG* config,const char* section);
/**
 * 获取section段名下指定key的值
 * @param section　段名,没有段名时可以为NULL
  * @param key 键名
...
char* ini_config_get_value_from_section(CONFIG_BTREE* section,const char* key);
